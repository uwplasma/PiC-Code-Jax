...
def func(x):
    ...
    for i in range(len(grid)):
        ext_E = ext_E.at[i].set(jnp.array(
            [-weight*1.6e-19*no_pseudoelectrons*A1*jnp.sin(k1*(grid[i]+dx/2)/L)/(k1*8.85e-12)
             -weight*1.6e-19*no_pseudoelectrons*A2*jnp.cos(k1*(grid[i]+dx/2)/L)/(k1*8.85e-12),0,0])
            )
...
start_time = time.time();sol = func([0.2,0.2,0.2]);print(f'For solution {sol} time taken is {time.time()-start_time:.2f}s')
...
x0=jnp.array([0.2,0.2,0.2])
max_number_function_evaluations = 15
max_number_iterations = 10
tolerance_to_stop_optimization = 1e-5

# Define the bounds: lower bound for A (x[0]) is 0.1, no specific constraints on the other variables
bounds = ([0.1, -float('inf'), 0.1], [float('inf'), float('inf'), float('inf')])

# Timing the execution
start_time = time.time()
# Perform least squares optimization with the added bounds
res_ls = least_squares(func, x0, bounds=bounds, verbose=2, ftol=tolerance_to_stop_optimization, max_nfev=max_number_function_evaluations)
# Extract the solution
sol_ls = res_ls.x[0], res_ls.x[1], res_ls.x[2]
