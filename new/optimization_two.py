...
def func(x):
    ...
    for i in range(len(grid)):
        ext_E = ext_E.at[i].set(jnp.array(
            [-weight*1.6e-19*no_pseudoelectrons*A*jnp.sin(k*(grid[i]+dx/2)/L)/(k*8.85e-12),0,0])
            )
...
start_time = time.time();sol = func([0.2,0.2]);print(f'For solution {sol} time taken is {time.time()-start_time:.2f}s')
...
bounds = ([0.1, -float('inf')], [float('inf'), float('inf')])
...
sol_bfgs = res_bfgs.x[0],res_bfgs.x[1]
