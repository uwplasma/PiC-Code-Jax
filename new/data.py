...
A_values = jnp.linspace(0.051, 0.1, 50)
#k_values = jnp.linspace(0.5*jnp.pi/L,6*jnp.pi/L, 24)
k_values= 0.5*jnp.arange(1, 17) * jnp.pi / L

# Create a grid of A and k values
A_grid, k_grid = jnp.meshgrid(A_values, k_values)

# Calculate the function values for each combination of A and k
func_values = jnp.zeros_like(A_grid)
for i in range(len(A_values)):
    for j in range(len(k_values)):
        x = [A_values[i], k_values[j]]
        # Update func_values using .at[] method
        func_values = func_values.at[j, i].set(func(x))  # Note: Transpose indexing due to meshgrid
